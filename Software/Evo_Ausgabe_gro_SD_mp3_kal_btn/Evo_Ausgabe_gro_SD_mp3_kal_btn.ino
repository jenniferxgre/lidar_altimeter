...
// MP3 Module variables
uint16_t distBuf = 0;
uint8_t announced = 0;

//Calibration button variable
...
void setup() {
  //Serial.begin(9600);     // Open serial port 0 (which corresponds to USB port), set data rate to 115200 bps !!!! RICHTIGE BAUDRATE
  ...
  if (digitalRead(buttonPin) == LOW) {
    ...
    if(myFile){
      ...
      myFile.write(buf[1]);
      // OPTION 2:
      //uint8_t fileBuf[2]; //Deklaration über Setup
      //fileBuf[0] = buf[0];
      //fileBuf[1] = buf[1];
      //myFile.write(fileBuf,2);
      myFile.close();
    }
  }
  else {
...
void loop() {
  ...
  if ((distBuf-distance > 50) && myDFPlayer.begin(mySoftwareSerial)){
    
    if ((announced != 1) && distance_m < 52.0 && distance_m > 48.0){
      myDFPlayer.playMp3Folder(1);
      announced = 1;
    }
    else if ((announced != 2) && distance_m < 42.0 && distance_m > 38.0){
      myDFPlayer.playMp3Folder(2);
      announced = 2;
    }
    else if ((announced != 3) && distance_m < 32.0 && distance_m > 28.0){
      myDFPlayer.playMp3Folder(3);
      announced = 3;
    }
    else if ((announced != 4) && distance_m < 22.0 && distance_m > 18.0){
      myDFPlayer.playMp3Folder(4);
      announced = 4;
    }
    else if ((announced != 5) && distance_m < 12.0 && distance_m > 9.0){
      myDFPlayer.playMp3Folder(5);
      announced = 5;
    }
    else if ((announced != 6) && distance_m < 6.0 && distance_m > 4.0){
...
uint16_t messen() {
  ...
  Wire.beginTransmission(SENSOR_ADDR);      // Transmit to TR1 (THIS IS THE I2C BASE ADDRESS, CHANGE HERE IN CASE IT IS DIFFERENT)
    ...
    /* Serial.println(buf[0]);             // Print buffer for debugging 
